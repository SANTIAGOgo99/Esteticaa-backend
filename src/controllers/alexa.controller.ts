...
const getActiveServices = async () => {
  ...
  return result.rows.map((service) => ({
    ...
    priceText: `$${formatMoney(service.price)} MXN`,
    meta: `${service.duration_minutes} min`,
  }));
};
...
const getActiveProducts = async () => {
  ...
  return result.rows.map((product) => ({
    ...product,
    type: 'product',
    description: `${product.brand || 'Producto'}${product.size ? `, ${product.size}` : ''}`,
    imageUrl: product.image_url,
    priceText: `$${formatMoney(product.price)} MXN`,
    meta: `${product.stock} disponibles`,
  }));
};
